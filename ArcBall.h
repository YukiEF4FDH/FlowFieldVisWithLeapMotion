...
// 8<--Snip here if you have your own math types/funcs-->8 
    ...
    typedef class ArcBall_t
    {
        protected:
			...
			void _MymapToSphere(const Leap::Vector* NewPt, Vector3fT* NewVec) const;
        ...
        public:
			...
			//Rotate gesture start
			void Myclick(const Leap::Vector* NewPt);

            //Mouse drag, calculate rotation
            void drag(const Point2fT* NewPt, Quat4fT* NewRot);
			void Mydrag(const Leap::Vector* NewPt, Quat4fT* NewRot);

        protected:
            Vector3fT   StVec;          //保存点击向量
            Vector3fT   EnVec;          //保存拖动向量
            GLfloat     AdjustWidth;    //鼠标范围宽
            GLfloat     AdjustHeight;   //鼠标范围高
